...
def plot_samples(sample_evaluation: Dict, filepath: string):
    ...
    for i, sample in enumerate(sample_evaluation):
        truth = sample['truth']
        pred = sample['pred'][0]
        plt.plot(truth, range(len(truth)), color=colors[i], linestyle='dashed', label='truth - sample ' + str(i+1))
        plt.plot(pred, range(len(pred)), color=colors[i], linestyle='solid', label='prediction - sample ' + str(i+1))
    plt.legend()
    plt.savefig(filepath)
    plt.clf()
